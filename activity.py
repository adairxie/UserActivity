...
def save_to_redis(ip, host, score, zone):
    if ip != "":
        try:
            ...
            logging.info('save ip credit information to redis failed, err %s' % str(e))
...
class UserActivity():
    ...
    def update_activity(self, records):
        ...
        try:
            ...
            logging.info(" write data to hdfs encounters an error, message:%s" % str(e))
        except Exception, e:
            logging.info("encounter error when read history data from hdfs, error message:%s" % str(e))
            return
    ...
    def Run(self, slc):
        for date in self.date_list:
            logging.info('beginning analysis %s ngx error log' % date)
            start = time.time()
            ...
            elapsed = done - start
            logging.info('%s analysis end, %.2f seconds elapsed' % (date, elapsed))
